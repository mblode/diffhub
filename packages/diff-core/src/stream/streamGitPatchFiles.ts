...
const getNextBoundarySearchIndex = (text: string, minimumIndex: number): number =>
  Math.max(minimumIndex, text.length - GIT_FILE_BOUNDARY_SCAN_OVERLAP);

const findNextGitFileBoundary = (text: string, fromIndex: number): number | undefined => {
...
const findLastCommitMetadataBoundary = (
  text: string,
  startIndex: number,
  endIndex: number,
): number | undefined => {
...
const getStreamedFileSplitIndex = (
  text: string,
...
const createGitPatchFileStreamParser = (): GitPatchFileStreamParser => {
  let buffer = "";
  let currentFileBoundaryIndex: number | undefined;
  let nextBoundarySearchIndex = 0;
  let sawFileBoundary = false;

  const takeAvailableFile = (): string | undefined => {
    if (currentFileBoundaryIndex === undefined) {
    ...
    for (;;) {
      ...
      if (nextBoundaryIndex === undefined) {
      ...
      nextBoundarySearchIndex =
        currentFileBoundaryIndex === undefined ? 0 : currentFileBoundaryIndex + 1;
...
const consumeAvailableStreamedFiles = async (
  parser: GitPatchFileStreamParser,
  onFileText: (fileText: string) => Promise<void>,
): Promise<void> => {
  let fileText: string | undefined;
  while ((fileText = parser.takeAvailableFile()) !== undefined) {
    await onFileText(fileText);
  }
};

export const streamGitPatchFiles = async (
...
export const getStreamedPatchMetadata = (fileText: string): string | undefined => {
  const diffBoundaryIndex = findNextGitFileBoundary(fileText, 0);
  if (diffBoundaryIndex === undefined || diffBoundaryIndex <= 0) {
    return undefined;
  }
