...
/**
 * The docs origin emits root-relative asset URLs (`/_next/...`). Those resolve
 * against blode.co, which fronts many projects and has no route for them, so
 * every stylesheet and script 404s and comes back as the HTML 404 page — which
 * the browser then refuses to execute for having the wrong MIME type.
 *
 ...
 */
const UPSTREAM_ASSET_PREFIX = "/_next/";
const PUBLIC_ASSET_PREFIX = `${PUBLIC_DOCS_PATH}${UPSTREAM_ASSET_PREFIX}`;

const isAssetSlug = (slug: string[]): boolean => slug[0] === "_next";
...
const ASSET_URL_PATTERN = /(["'(])\/_next\//g;
